...
```bash
./build-linux-cross-sdk.sh swift-5.10-RELEASE ubuntu-noble
```
...
```bash
sudo tar -xf swift-5.10-RELEASE-debian-bookworm-armv7-sdk.tar.gz -C /opt
```

Then, from a Swift package, use the `--destination` paramter to cross-compile it for the target:

```bash
swift build --destination /opt/swift-5.10-RELEASE-debian-bookworm-armv7/debian-bookworm.json
```
